//! Test compatibility with baseline keys generated by malai keygen
//! 
//! This test ensures that keys generated by baseline kulfi-utils/malai
...
fn test_baseline_secret_keys_load_correctly() {
    ...
    for (file_path, expected_hex, expected_id52) in baseline_keys {
        ...
        assert_eq!(key_hex, expected_hex, "Key file content doesn't match expected for {}", file_path);
        ...
        assert_eq!(display_hex, expected_hex, "Display encoding changed for {}", file_path);
        
        // Test that ID52 matches what baseline produced
        let public_key = secret_key.public();
        let id52 = kulfi_utils::public_key_to_id52(&public_key);
        assert_eq!(id52, expected_id52, "ID52 encoding changed for {}", file_path);
        ...
        assert_eq!(id52, roundtrip_id52, "ID52 roundtrip failed for {}", file_path);
...
fn test_secret_key_hex_format() {
    ...
    assert!(display.chars().all(|c| c.is_ascii_hexdigit() && !c.is_ascii_uppercase()),
...
fn test_id52_format() {
    ...
    let decoded = data_encoding::BASE32_DNSSEC.decode(test_id52.as_bytes())
...
fn test_generate_and_verify_new_key() {
    ...
    let (id52, secret_key) = kulfi_utils::generate_secret_key()
        .expect("Should generate key successfully");
