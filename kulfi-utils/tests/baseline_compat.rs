...
fn test_baseline_keys_load_correctly() {
    ...
    for (file_path, expected_hex, expected_id52) in test_data {
        ...
        assert_eq!(key_hex, expected_hex, "Key file content doesn't match expected");
        ...
        assert_eq!(id52, expected_id52, 
                   "ID52 mismatch for {}! Expected: {}, Got: {}", 
                   file_path, expected_id52, id52);
        ...
        println!("  ✓ {} loads correctly, ID52 matches", file_path);
    }
}
...
fn test_signature_with_baseline_key() {
    ...
    let secret = kulfi_utils::SecretKey::from_str(&key_hex)
        .expect("Failed to parse baseline key");
...
fn test_public_key_derivation() {
    ...
    // Test ID52 roundtrip
    let parsed = kulfi_utils::PublicKey::from_id52(expected_id52)
